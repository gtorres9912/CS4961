      ...
      */
      case MM_CMD_MID:
         ...
         switch (CommandCode)
         {
            ...
            case MM_DETECTSBE_CC: {
               // Read memory from SBEI_AppData 
               uint8 sbei = MM_readSBEIMem(msg);
               CFE_EVS_SendEvent(MM_DETECTSBE_EID, CFE_EVS_INFORMATION, "DETECTSBE Command: SBEI data -> %X", sbei);
               // Read memory from SBED_AppData
               uint16 sbed = MM_readSBEDMem(msg);
               CFE_EVS_SendEvent(MM_DETECTSBE_EID, CFE_EVS_INFORMATION, "DETECTSBE Command: SBED data -> %X", sbed);
               // Check if a bit has been flipped
               MM_isChanged(msg, sbei, sbed);
               break;
            }
